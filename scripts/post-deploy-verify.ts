...
type SupabaseQueryLike = PromiseLike<{ data: unknown; error: QueryErrorLike }> & {
    eq: (...args: unknown[]) => SupabaseQueryLike;
    in: (...args: unknown[]) => SupabaseQueryLike;
    like: (...args: unknown[]) => SupabaseQueryLike;
    is: (...args: unknown[]) => SupabaseQueryLike;
...
async function querySystemView<T>(
    ...
    view: string,
    columns: string,
    filter?: (q: SupabaseQueryLike) => SupabaseQueryLike
): Promise<{ data: T[] | null; error: Error | null }> {
    let q = supabase.from(view).select(columns) as unknown as SupabaseQueryLike;
    if (filter) q = filter(q);
    const { data, error } = await q;
    return {
        data: data as T[] | null,
        error: error ? new Error(String(error.message ?? error)) : null,
    };
}
...
async function httpGetPublicSlots(url: string, description: string): Promise<void> {
    try {
        ...
        const isValidSlots = Array.isArray(slots) && slots.every((slot) => typeof slot === "string");
        if (!isValidSlots) {
