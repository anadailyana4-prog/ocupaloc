...
export async function handleSignoutRequest(request: NextRequest, deps: SignoutDeps = defaultDeps) {
  if (!isAllowedSignoutRequest(request)) {
    deps.report("auth", "signout_csrf_rejected", "Invalid origin", {
  ...
  const supabase = deps.createClient(url, anon, {
    cookies: {
      getAll() {
