-- PR 2: Atomic booking with idempotency support
-- Prevents double-bookings via SERIALIZABLE transaction + FOR UPDATE locks
-- Adds request deduplication table for safe idempotency key handling

-- Request deduplication table for idempotency
-- Safe concurrency: unique(idempotency_key) prevents duplicate in-flight requests
CREATE TABLE IF NOT EXISTS public.request_deduplication (
  ...
  CONSTRAINT request_deduplication_key_unique UNIQUE(idempotency_key)
);

CREATE INDEX IF NOT EXISTS idx_request_dedup_expires_at 
  ON public.request_deduplication(expires_at);

-- Cleanup expired idempotency records (can be called by cron)
CREATE OR REPLACE FUNCTION cleanup_expired_idempotency_keys()
RETURNS void AS $$
BEGIN
  DELETE FROM public.request_deduplication WHERE expires_at < NOW();
END;
$$ LANGUAGE plpgsql;

-- Main atomic booking RPC
-- Runs in SERIALIZABLE transaction to prevent race conditions
-- Checks: profesionist exists, service exists, subscription valid, client not blocked, slot free
-- Returns: (success, programare_id, error_code, error_message)
...
BEGIN
  ...
  -- 3. Check entitlement (subscription active or in trial)
  ...
  IF v_subscription_status IS NULL THEN
    ...
    ELSE
      ...
      RETURN;
    END IF;
  ELSIF v_subscription_status IN ('active', 'trialing') THEN
  ...
  -- 7. Slot availability check with FOR UPDATE lock
  SELECT EXISTS(
    SELECT 1 FROM public.programari
    WHERE profesionist_id = v_prof_id
      AND status != 'anulat'
      AND data_start < p_slot_start + (v_service_record.durata_minute || ' minutes')::INTERVAL
  ...
  -- 8. Calculate data_final using exact logic from slots.ts
...
COMMENT ON FUNCTION book_appointment_atomic IS 
  'Atomic booking in SERIALIZABLE transaction. Prevents double-bookings via FOR UPDATE locks.';
