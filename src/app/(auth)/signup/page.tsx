...
export default function SignupPage() {
  ...
  const [activity, setActivity] = useState<(typeof ACTIVITATI)[number]>(ACTIVITATI[0]);
  ...
  async function onCreateAccount() {
    ...
    if (data.session) {
      const boot = await bootstrapTenantAfterSignup({ orgName: businessName, slug });
      if (!boot.ok) {
        setIsSubmitting(false);
            ...
            <section className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <select
                    value={activity}
                    onChange={(event) => setActivity(event.target.value as (typeof ACTIVITATI)[number])}
              ...
              <Dialog open={csvOpen} onOpenChange={setCsvOpen}>
                <DialogTrigger asChild>
                  <button type="button" className="text-sm text-primary underline-offset-4 hover:underline">
                    Ai deja clienți în Fresha sau Excel? Importă-i
                  </button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
